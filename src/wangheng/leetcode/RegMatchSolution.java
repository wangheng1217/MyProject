...
public class RegMatchSolution {
    public boolean isMatch0(String s, String p) {
        ...
        List<Boolean> hasStar = new ArrayList<Boolean>();
            ...
            for (int j = 0; j <= pList.size(); j++) {
                if (i == 0) {
                    if (j == 0) dp[i][j] = true;
                        ...
                        dp[i][j] = dp[i][j-1] && hasStar.get(j-1);
                    }
                } else if (j == 0) {
                    dp[i][j] = false;
                } else {
                    if (!hasStar.get(j-1)) {
                        dp[i][j] = dp[i-1][j-1] && isMatch(s.charAt(i-1), pList.get(j-1));
                    } else {
                        int k = i;
                        while (k >= 0) {
                            if (dp[k][j-1]) {
                            ...
                            if (!isMatch(s.charAt(k-1), pList.get(j-1))) {
                                dp[i][j] = false;
                                break;
                            }
                            k--;
